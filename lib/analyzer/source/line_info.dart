...
final class CharacterLocation {
  ...
  bool operator ==(Object other) {
    return other is CharacterLocation && lineNumber == other.lineNumber && columnNumber == other.columnNumber;
  }
}
...
class LineInfo {
  ...
  factory LineInfo.fromContent(String content) {
    const int slashN = 0x0A;
    const int slashR = 0x0D;

    List<int> lineStarts = <int>[0];
    int length = content.length;

    for (int i = 0; i < length; i++) {
      int unit = content.codeUnitAt(i);
  ...
  int getOffsetOfLine(int lineNumber) {
    if (lineNumber < 0 || lineNumber >= lineCount) {
      throw ArgumentError('Invalid line number: $lineNumber; must be between 0 and ${lineCount - 1}');
    }
