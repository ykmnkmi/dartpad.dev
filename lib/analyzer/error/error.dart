...
import 'package:dartpad/analyzer/src/error/error_code_values.g.dart';
import 'package:dartpad/analyzer/src/generated/java_core.dart';

export 'package:_fe_analyzer_shared/src/base/errors.dart' show ErrorCode, ErrorSeverity, ErrorType;
export 'package:dartpad/analyzer/src/error/error_code_values.g.dart';

/// The lazy initialized map from [ErrorCode.uniqueName] to the [ErrorCode]
/// instance.
final HashMap<String, ErrorCode> _uniqueNameToCodeMap = _computeUniqueNameToCodeMap();
...
HashMap<String, ErrorCode> _computeUniqueNameToCodeMap() {
  HashMap<String, ErrorCode> result = HashMap<String, ErrorCode>();
...
final class AnalysisError implements Diagnostic {
  /// An empty array of errors used when no errors are expected.
  static const List<AnalysisError> noErrors = <AnalysisError>[];

  /// A [Comparator] that sorts by the name of the file that the [AnalysisError]
  /// was found.
  static Comparator<AnalysisError> fileComparator = (AnalysisError error1, AnalysisError error2) {
    return error1.source.shortName.compareTo(error2.source.shortName);
  };

  /// A [Comparator] that sorts error codes first by their severity (errors
  /// first, warnings second), and then by the error code type.
  static Comparator<AnalysisError> errorCodeComparator = (AnalysisError error1, AnalysisError error2) {
    ErrorCode errorCode1 = error1.errorCode;
    ErrorCode errorCode2 = error2.errorCode;
  ...
  AnalysisError.forValues({
    ...
    List<DiagnosticMessage> contextMessages = const <DiagnosticMessage>[],
    this.data,
