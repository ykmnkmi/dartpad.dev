...
final class FileSource implements Source {
  ...
  static FileReadMode fileReadMode = (String string) {
    return string;
  };
  ...
  FileSource(this.file, [Uri? uri])
      : uri = uri ?? file.toUri(),
        id = _idTable.putIfAbsent('${uri ?? file.toUri()}@${file.path}', () => _idTable.length);
  ...
  TimestampedData<String> get contentsFromFile {
    return TimestampedData<String>(file.modificationStamp, fileReadMode(file.readAsStringSync()));
  }

  @override
  String get fullName {
    return _absolutePath ??= file.path;
  }
